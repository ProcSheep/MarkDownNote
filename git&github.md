...
>
- ==11.移除文件==
  - 把工作区和git仓库同时移除 `git rm -f index.js`; 注意：如果是一个目录（文件夹），可以执行下面命令递归取消文件夹内所有文件的跟踪`git rm -r --cached node_modules`
  - 只移除git库 `git rm --cached index.js`
  - ==对应的状态显示为**D(delete),在下次提交时会被删除**,并不再被git跟踪==
>
- ==**12.忽略文件.gitignore**==
  - 既不想加入git也不想被标记未跟踪U,在目录下创建`.gitignore`配置文件,使用vscode编辑文件内容,==注意:已经被忽略的文件必须是未被提交的文件,如果之前已经提交到git库中了,那么就一定会被跟踪,需要先把它从git库中删除,`git rm --cached 文件相对于执行终端的路径`,然后再提交,这样才会取消跟踪,**所以这个操作最好是在提交前进行规避追踪**,配置好gitignore文件后,进行`git add .`保存到暂存区即可生效,提交后也生效==
  - ==规范语法==
    - #开头是注释
    - /结尾为目录
    - /开头防止递归(防止向上一级目录递归)
...
- ==2.常用的代码指令(注意正常创建好本地和远程github后再添加tag)==
  - 1.==提交后commit,对此次提交添加tag标签== 
    ...
    - 也可以给予历史添加: `git tag -a v1.0.0 <commit-id> -m "基于...提交发布v1.0.0"`,通过`git log`可以查看提交的历史commit-id
  - 2.查看所有的tag `git tag`
  ...
  - 6.==回退到某个tag版本(仅可读)==
    - `git checkout v1.0.0`,==回退后要修改内容,需要创建新的分支修改,不要影响到main分支里的源代码==
  - 7.==**规范：语义化版本标签（SemVer）**==
    - 推荐遵循「语义化版本 2.0」规范，格式：v<主版本>.<次版本>.<补丁版本>-<预发布>-<构建信息>示例：
    - v1.0.0：正式发布版本；
    - v1.1.0：新增功能（次版本升级）；
    - v1.0.1：修复 bug（补丁版本升级）；
    - v2.0.0-beta：预发布测试版；
    - v1.2.3-alpha.1：alpha 测试版（内部测试）。
- ==**4.分支**==
- ==1.什么是分支?==
