...
- Node中的代理proxy
  ...
  ```
  > ==细节:== /api 在这里是一个路径标识符，作用是让服务器知道：当客户端请求的 URL 以 /api 开头时，才需要使用这个代理规则进行转发。
  1.当客户端发送请求，比如 `http://你的前端服务器/api/user/demo` 时：服务器看到 URL 以 /api 开头，就会触发这个代理规则
  接着进行两件事：
  2.1 把请求转发到 target 配置的目标服务器（这里是 `http://localhost:8000`）
  2.2 同时通过 pathRewrite 把 URL 中的 /api 去掉，所以实际转发的地址是 `http://localhost:8000/user/demo`
