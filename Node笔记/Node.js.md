...
- ==readdir: 读取文件夹或文件的信息,深层嵌套文件可以递归读取==
  ...
  ```
- ==参数: withFileType: 在读取文件后可以返回文件基本信息==
  ```js
    // 异步读取
    const fs = require('fs').promises;

    const items = await fs.readdir('./test-dir', { withFileTypes: true });

    for (const item of items) {
      console.log(`名称: ${item.name}`);
      console.log(`是否为文件: ${item.isFile()}`);       // 如：true
      console.log(`是否为目录: ${item.isDirectory()}`); // 如：false
      console.log('---');
    }
  ```
- 参数补充: 
  简单读取当前目录 → 默认配置（{}）即可。
  区分文件 / 目录 → 加 withFileTypes: true。
  遍历所有子目录 → 加 recursive: true。递归性能消耗大,谨慎
  处理特殊编码文件名 → 用 encoding: 'buffer'。
### fs-重命名(了解)
- ==rename: 重命名文件夹或文件的名字==
  ```js
